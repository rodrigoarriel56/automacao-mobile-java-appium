...
chai.use(function(chaia, util) {
  chaia.Assertion.addMethod('sortedByDate', function(format, asc) {
    const obj = this._obj;
    return chaia
      .expect(
        obj,
        `expected ${inspect(obj)} to be sorted by date with format '${format}' in ${
          asc ? 'ascending' : 'descending'
        } order`
      )
      .to.be.sorted((prev, next) => {
        const previousDate = moment(prev, format, true);
        const nextDate = moment(next, format, true);
        if (!previousDate.isValid() || !nextDate.isValid()) {
          return true;
        }
        if (asc) {
          return previousDate.isSameOrAfter(nextDate);
        } else {
          return previousDate.isSameOrBefore(nextDate);
        }
      });
  });
});
...
class chaiWrapper extends Helper {
  ...
  /**
   * @param {*} targetArrayData
   * @param {*} customErrorMsg
   * @returns {*}
   */
  assertSortedByDate(targetArrayData, customErrorMsg) {
    return expect(targetArrayData, customErrorMsg).to.be.sortedByDate();
  }

  assertIsBetween(comparisonValue, firstValue, lastValue) {
    return expect(comparisonValue.toDateString()).to.be.within(firstValue, lastValue);
  }

  assertPeriodEquals(initialValue, comparisonValues, periodSelected) {
    periodInitial = initialValue.toDateString();
    periodFinal = initialValue.toDateString() + periodSelected.toDateString(); // todo: TESTAR ISSO
    value = comparisonValues.toDateString();

    return expect(value).to.be.within(periodInitial, periodFinal);
  }

  assertNotText(data, value) {
    return expect(data).to.not.include(value);
  }

  assertText(data, value) {
    return expect(data).to.include(value);
  }

  assertCurrencyValue(actualValue, expectedValue) {
    expectedValue = parseFloat(expectedValue).toLocaleString('pt-br', {
      style: 'currency',
      currency: 'BRL',
